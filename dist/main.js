...
function createWindow() {
    ...
    win.loadURL('data:text/html,<h1 style="color:white;font-size:20px;">Press the key "P".</h1>');
}
async function captureAndRespond() {
    try {
        ...
        if (!question) {
            ...
            win.loadURL(`data:text/html,<h1 style="color:white;">No question found in OCR text.</h1>`);
            return;
        }
        ...
        if (response.text) {
            ...
            win.loadURL(`data:text/html,<body style="background:transparent;color:white;font-size:16px;">${response.text}</body>`);
        }
        else {
            console.error("AI Failed to respond: empty text in response");
            win.loadURL(`data:text/html,<h1 style="color:white;">AI Failed to respond.</h1>`);
        }
        console.log("=== captureAndRespond finished ===");
    }
    catch (error) {
        win.loadURL(`data:text/html,<h1 style="color:white;">An error occurred. Check console.</h1>`);
